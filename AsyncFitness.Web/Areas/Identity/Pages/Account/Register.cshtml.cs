...
namespace AsyncFitness.Web.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private readonly IUserStore<AsyncFitnessUser> _userStore;
        ...
        private AsyncFitnessUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<AsyncFitnessUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(AsyncFitnessUser)}'. " +
                    $"Ensure that '{nameof(AsyncFitnessUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<AsyncFitnessUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<AsyncFitnessUser>)_userStore;
        }
    }
}
